...
def get_season_total(home_stat_id, away_stat_id, total_stat_id, season):
  ...
  total_insert = """INSERT INTO player_country_stats(stat_id, player_id, country_id, season_id, sum, count, average)
                         SELECT %s, player_id, country_id, season_id, SUM(sum), SUM(count), CAST(SUM(sum) AS FLOAT) / SUM(count)
                           FROM player_country_stats
                          WHERE (stat_id = %s OR stat_id = %s)
                            AND season_id = ?
                          GROUP BY player_id, country_id"""
...
def sum_all_seasons(all_seasons_const):
  ...
  total_insert = """INSERT INTO player_country_stats(stat_id, player_id, country_id, season_id, sum, count, average)
                         SELECT stat_id, player_id, country_id, '%s', SUM(sum), SUM(count), CAST(SUM(sum) AS FLOAT) / SUM(count)
                           FROM player_country_stats
                          GROUP BY stat_id, player_id, country_id"""
...
stat_insert = """INSERT INTO player_country_stats(stat_id, player_id, country_id, season_id, sum, count, average)
                      SELECT %s, player_id, team.country_id, season_id, SUM(%s), COUNT(*), CAST(SUM(%s) AS FLOAT) / COUNT(*)
                        FROM prediction, game, team
...
# calculate stats and insert into player_country_stats

cur.execute('DELETE FROM player_country_stats')
...
# insert stat descriptions into player_country_stats_description

description_insert = """INSERT INTO player_country_stats_description
                             VALUES(?, ?, ?, ?)"""

cur.execute('DELETE FROM player_country_stats_description')
...
cur.execute(description_insert, (prediction_success_home_stat_id, 'prediction_success_country_home', sort_desc, 'Avg. prediction points scored home'))
cur.execute(description_insert, (prediction_success_away_stat_id, 'prediction_success_country_away', sort_desc, 'Avg. prediction points scored away'))
cur.execute(description_insert, (prediction_success_total_stat_id, 'prediction_success_country', sort_desc, 'Avg. prediction points scored'))

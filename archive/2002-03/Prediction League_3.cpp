...
void main()
{
	...
	FILE *standings_new; // new overall standings
	...
	while (choice != 'J')
	{
		...
		printf("h) Update standings (only after g) !)\n");
		...
		switch (choice)
		{
			...
			case 'H': // Update standings
				...
				standings_new = fopen("standings_new.txt", "w");
				if ((standings = fopen("standings.txt", "r")) == NULL)
					standings = fopen("standings.txt", "w+");
				fprintf(standings_new, "          Competitor:                    Pts: Res:\n");
				line = (char *)malloc(80*sizeof(char));
				...
				for (i=0; i<numberOfPlayers; i++)
				{
					...
					if (temp)
						if (j != 0)
							fprintf(standings_new, "%2d. (%+3d) %-30s %3d  %2hd\n", i+1, j, sortStandings[i].name, sortStandings[i].points, sortStandings[i].spotOn);
						else
							fprintf(standings_new, "%2d. ( - ) %-30s %3d  %2hd\n", i+1, sortStandings[i].name, sortStandings[i].points, sortStandings[i].spotOn);
					else
						fprintf(standings_new, "%2d. (new) %-30s %3d  %2hd\n", i+1, sortStandings[i].name, sortStandings[i].points, sortStandings[i].spotOn);
				}
				free(line);
				free(line2);
				fclose(standings);
				fclose(standings_new);
				remove("standings.txt");
				rename("standings_new.txt", "standings.txt");
				free(sortStandings);
			...
			case 'I': // Print database
				...
				while (fread(&newData, sizeof(newData), 1, database))
					printf("%-30s %3d %2hd\n", newData.name, newData.points, newData.spotOn);
				getchar();
				break;
			case 'J': // Quit
				break;
